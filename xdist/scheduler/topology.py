...
class TopologyScheduling:
    ...
    def __init__(self, config, log=None):
        ...
        if log is None:
            self.log = Producer('topologysched')
        else:
            self.log = log.topologysched
    ...
    def _assign_work_unit(self, node):
        ...
        if not node.topology:
            ...
            node.topology = scope.split('::')[0]
        else:
            # Grab a unit of work
            for sc in self.workqueue:
                sco = sc.split('::')[0]
                if sco == node.topology:
                    scope = sc
                    work_unit = self.workqueue[sc]
                    del self.workqueue[sc]
                    node.topology = sco
                    break
        ...
        pprint.pprint(scope)
        print('work_unit: ')
        pprint.pprint(work_unit)
        print('topology: ')
        pprint.pprint(node.topology)
